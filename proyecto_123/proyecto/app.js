...
app.get("/", function(req,res){
	res.render("index.jade", {title : "Chat Triviador"});
});
...
io.sockets.on('connection', function(socket) 
{
	...
	socket.on("loginUser", function(username)
	{
		...
		socket.broadcast.emit("refreshChat", "conectado", "El usuario " + socket.username + " se ha unido a la Partida.");
	...
	socket.on("disconnect", function()
	{
		...
		socket.broadcast.emit("refreshChat", "desconectado", "El usuario " + socket.username + " se ha dejado la Partida.");
	});
});
